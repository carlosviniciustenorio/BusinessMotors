namespace BusinessMotors.Application.Queries
{
    ...
    {
        public sealed record Anuncios(int take, string? estado, decimal? precoInicio, decimal? precoFim, int? kmInicio, int? kmFim, int? anoModeloInicio, int? anoModeloFim, int? idMarca, int? idModelo, int? idVersao, int skip = 0, string role = "", string cidade = "") : IRequest<List<AnunciosResponse>>;
